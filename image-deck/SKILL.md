---
name: image-deck
description: "Create full-slide PPT/deck pages by calling Codex's built-in image_gen capability once per slide while maintaining strong visual consistency across all pages. Use only when the user wants every slide/page of a PPT, presentation, deck, carousel, or keynote-style artifact generated as complete raster images with Codex image_gen, including all visible text inside the generated image itself. Before generation, show complete prompt groups inline in the chat, up to 8 slides per group, for user review. Default to 图文并茂 PPT pages where each slide combines appropriate in-image text and visuals according to its role, not just a decorative background. Do not use for ordinary editable PPT decks, normal presentation design, small deck edits, text-overlay workflows, prompt-pack-only workflows, or chart/table-heavy business decks unless the user explicitly wants each page generated as one finished image through image_gen."
---
