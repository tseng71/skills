...
Text rule: all generated slide content text must be generated inside this image; do not leave blank title areas for later editing. Do not generate the page number in the image; it is added later as a native PowerPoint slide-number field. Match text density to slide role and selected content-density mode. Cover slides have strict text rules: main title plus optional subtitle only, unless the user explicitly asks for additional cover text. Normal content slides should not feel underwritten for the selected mode.
...
Preserve: aspect ratio, palette, lighting, margins, typography mood, native slide-number field style, graphic language, and role-specific layout.
```
