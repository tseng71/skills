...
Automatically communicate with the user in the language they are using in the current conversation. If the user writes in Chinese, ask questions, explain progress, and report results in Chinese. If the user writes in English, use English. If the user switches language mid-task, follow the newest user message unless they explicitly requests a different communication language.

Keep communication language separate from deck language. Always ask the deck language during intake because the user may chat in one language while wanting the PPT content in another.
