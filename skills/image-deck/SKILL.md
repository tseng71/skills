---
name: image-deck
description: Create full-slide image decks by calling Codex's built-in image_gen capability once per slide while maintaining strong visual consistency across all pages. Use only when the user wants every slide/page of a PPT, presentation, deck, carousel, or keynote-style artifact generated as complete raster images with Codex image_gen, including all visible text inside the generated image itself. Do not use for ordinary editable PPT decks, normal presentation design, small deck edits, text-overlay workflows, prompt-pack-only workflows, or chart/table-heavy business decks unless the user explicitly wants each page generated as one finished image through image_gen.
---
...
- Every slide's visible title, labels, and short copy are inside the generated image itself, not overlaid later.
...
- **Language:** Ask whether the deck should be Chinese, English, bilingual, or mostly visual.
...
### 3. Use full-image mode only

All visible content must be generated inside each slide image. This includes the title, small labels, page number, section tag, captions, and any short copy. Keep text sparse so `image_gen` has a realistic chance of producing readable results.
...
- title, preferably under 8 words
- one-sentence message
- visual object, such as scene, diagram, chart metaphor, portrait, product view, map, or timeline
- text load: none, light, medium

For image-only decks, avoid dense tables, long paragraphs, exact financial disclosures, and small body copy. Convert complex content into simple visual claims with short generated titles and labels.
...
- grid, title zone, footer/page marker zone
...
3. exact allowed visible text to generate inside the image, kept short
...
- missing title/label text that was supposed to be inside the generated image
...
- Keep generated in-image text short: title plus 1-3 labels.
