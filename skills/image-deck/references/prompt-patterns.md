...
Inner-page layout grid: <role-appropriate title/claim zone>, <text/callout zone>, <main visual zone>, generous safe margins.
...
Allowed visible text only:
- <list exact slide text here: title/claim, captions, bullets, callouts, labels, or section tag as appropriate to this slide role>
Text richness/content density: <information-rich / balanced / concise>
...
Composition: <where generated text belongs, where the main visual goes, and how labels/callouts attach to the visual. For cover slides, use only the main title plus optional subtitle unless the user explicitly asked for more cover text. For content slides, allow naturally generated supporting detail text when it makes the slide richer and more complete. Keep style consistent without copying another slide's background>.
...
Consistency rule: keep the same palette, lighting, margin, typography mood, and graphic language as the approved visual bible and other prompt groups.
```
...
Keep the title area cleaner, reduce decorative clutter, and preserve the same palette as the approved master sample.
```
...
Preserve: aspect ratio, palette, lighting, margins, typography mood, graphic language, and role-specific layout.
```
